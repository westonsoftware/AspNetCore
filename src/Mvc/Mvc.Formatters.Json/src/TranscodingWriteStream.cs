...
namespace Microsoft.AspNetCore.Mvc.Formatters.Json
{
    internal sealed class TranscodingWriteStream : Stream
    {
        ...
        public TranscodingWriteStream(Stream stream, Encoding sourceEncoding, Encoding targetEncoding)
        {
            _stream = stream;
        ...
        public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
        {
            ...
            var maxChars = _sourceEncoding.GetMaxCharCount(count);
            var charBuffer = _charPool.Rent(maxChars);

            try
            {
                var writtenChars = _sourceEncoding.GetChars(buffer.AsSpan(offset, count), charBuffer);
